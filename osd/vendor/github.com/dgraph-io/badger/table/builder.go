...
func NewTableBuilder(tableSize int64) *TableBuilder {
	return &TableBuilder{
...
func (b *TableBuilder) addHelper(key []byte, v y.ValueStruct) { //wuxingyi: 将key/value添加到到table中,还添加了相关元数据。
...
func (b *TableBuilder) Add(key []byte, value y.ValueStruct) error {
	//wuxingyi: 非常有趣的是，他们是往一个block里放置固定的restartInterval=100个key,通过counter来计数的。
