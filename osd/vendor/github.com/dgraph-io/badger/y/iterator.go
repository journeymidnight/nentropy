...
func (eh elemHeap) Less(i, j int) bool {
	...
	// The keys are equal. In this case, lower nice take precedence. This is important. //wuxingyi: 这里的nice，就是看memtable加入的时间来判定，越nice则说明加入的越晚。
...
// initHeap checks all iterators and initializes our  and array of keys.
// Whenever we reverse direction, we need to run this.
// wuxingyi: 每次变更方向时，都需要进行调用。
...
func (s *MergeIterator) Next() {
	...
	for len(s.h) > 0 {
		smallest = s.h[0].itr
		if !smallest.Valid() {
			heap.Pop(&s.h) //wuxingyi: 这里是移除整个Iterator，因为这个Iterator里面已经没有元素了。
			continue
		}
