...
type Manifest struct {
	Levels []LevelManifest          //wuxingyi: 存储每个level中的所有table
	Tables map[uint64]TableManifest //wuxingyi: 获得指定table所在level，所以看起来有点冗余。
...
// file we append to. //wuxingyi: 难道仅仅是充当了一个log文件？
...
func (mf *manifestFile) addChanges(changes protos.ManifestChangeSet) error {
	...
	// Rewrite manifest if it'd shrink by 1/10 and it's big enough to care //wuxingyi: 如果删除次数过多，则重新写一个新的manifest文件。
...
func helpRewrite(dir string, m *Manifest) (*os.File, int, error) {
	...
	if err := fp.Sync(); err != nil { //wuxingyi: manifest写入是手工调用Sync进行的，因为有一个rename操作，还需要sync一下文件所在的目录。
		fp.Close()
		return nil, 0, err
	}
