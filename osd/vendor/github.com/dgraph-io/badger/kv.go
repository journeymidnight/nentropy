...
type KV struct {
	...
	imm       []*skl.Skiplist // Add here only AFTER pushing to N.
	opt       Options
...
func NewKV(optParam *Options) (out *KV, err error) {
	...
	//wuxingyi: 这里就设计到一个先有鸡还是先有蛋的问题，在系统系统的时候，是不可能有table的，因此只有把manifest文件中看到的sst文件和其所在的level load到内存中，才能有内存中的Table。
...
func (s *KV) yieldItemValue(item *KVItem, consumer func([]byte)) error {
	...
	if (item.meta & BitValuePointer) == 0 {
		val := item.slice.Resize(len(item.vptr)) //wuxingyi: 这个是说value直接存在lsm上
		copy(val, item.vptr)
		consumer(val)
		return nil
	}
...
func (s *KV) get(key []byte) (y.ValueStruct, error) {
	...
	for i := 0; i < len(tables); i++ {
		...
		if vs.Meta != 0 || vs.Value != nil {
			return vs, nil //wuxingyi: 首先是从memtable找，找不到再在lsm书上找,这里找到的有可能直接是value，也可能是指向vlog的指针?
		}
	}
	return s.lc.get(key)
}
...
func (s *KV) Get(key []byte, item *KVItem) error {
	...
	item.key = key
	item.kv = s
	item.vptr = vs.Value //wuxingyi: 注意,通过Get接口只能获得一个指向value的指针啊, 并不是真正的value。
		...
		if s.shouldWriteValueToLSM(*entry) { // Will include deletion / tombstone case.
			...
			s.mt.Put(entry.Key,
				y.ValueStruct{
					...
					Meta:       entry.Meta | BitValuePointer, //wuxingyi： 这个标记说明kv是存在vlog上的。
					UserMeta:   entry.UserMeta,
					CASCounter: entry.casCounter})
		}
	}
...
func (s *KV) writeRequests(reqs []*request) error {
	...
	for _, req := range reqs {
		...
		for i, e := range req.Entries {
			e.casCounter = counterBase + uint64(i) //wuxingyi: 这里我们改变了casCounter的值，这个就有点像是rocksdb中的sequence了啊，每一个entry都拥有了一个sequence。
		}
	}
...
func (s *KV) sendToWriteCh(entries []*Entry) []*request {
	...
	for _, entry := range entries {
		...
		if size >= s.opt.maxBatchSize { //wuxingyi: 单个request的size如果过大，则拆分成多个request分别丢到channel。
			s.writeCh <- b
...
// ensureRoomForWrite is always called serially. //wuxingyi: 确保有足够的空间写lsm，如果没有就创建新的memtable，同时将前一个设置为immutable.
...
func writeLevel0Table(s *skl.Skiplist, f *os.File) error {
	...
	_, err := f.Write(b.Finish()) //wuxingyi: 以sync方式写入
	return err
}

type flushTask struct {
	mt   *skl.Skiplist
	vptr valuePointer //wuxingyi: vptr是标志已经被sync的vlog位置。
}
...
func (s *KV) flushMemtable(lc *y.Closer) error {
	...
	for ft := range s.flushChan {
		...
		if !ft.vptr.IsZero() {
			...
			// wuxingyi: 此处存疑啊，为什么还没有flush到文件，就更新head呢？这样是不是不安全？此时的Put操作，实际上只能放到vlog跟lsm的mutable memtable中，
			// wuxingyi: 起到的作用只能是可持久化，replay时，不一定能读到这个最新的head，当然这本身也不会有特别大的问题，无非就是多replay一点而已。
			ft.mt.Put(head, y.ValueStruct{Value: offset, CASCounter: s.lastCASCounter()}) //wuxingyi: 在这里更新head到lsm中,这里用的是更底层的接口，直接往lsm里送。这个丢了也没关系，只是多replay一部分而已。
		}
		fileID := s.lc.reserveFileID()
		fd, err := y.CreateSyncedFile(table.NewFilename(fileID, s.opt.Dir), true) // wuxingyi: add new sst file
		if err != nil {
			return y.Wrap(err)
		}
