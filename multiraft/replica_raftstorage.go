...
func entries(
	...
	if err := e.IterateEntries(ctx, e, expectedIndex, hi, scanFunc); err != nil {
		return nil, err
	}
...
func encodeRaftCommandV1(commandID CmdIDKey, command []byte) []byte {
	return encodeRaftCommand(raftVersionStandard, commandID, command)
}

func encodeRaftCommandV2(commandID CmdIDKey, command []byte) []byte {
