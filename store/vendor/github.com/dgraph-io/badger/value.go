...
func (vlog *valueLog) rewrite(f *logFile) error {
	...
	fe := func(e Entry) error {
		...
		vs, err := vlog.kv.get(e.Key) //wuxingyi: 这里要重新在lsm中查找一次，因为可能被替换了。
		if err != nil {
			return err
		}

		if (vs.Meta & BitDelete) > 0 { //wuxingyi: key已经被删除了，那么就不需要重新恢复了。
			return nil
		}
		if (vs.Meta & BitValuePointer) == 0 { //wuxingyi: value比较短，本来就存在lsm中
			return nil
		}
		...
		var vp valuePointer
		vp.Decode(vs.Value)
		//wuxingyi: 这两种情况说得都是已经被update了的情况。
		if vp.Fid > f.fid {
			return nil
		}
...
type valuePointer struct { //wuxingyi: 用于标识一对kv pair所处的vlog及其所在的vlog中的具体位置
	Fid    uint32
	Len    uint32
	Offset uint32
}
...
func (vlog *valueLog) write(reqs []*request) error {
	...
	for i := range reqs {
		...
		for j := range b.Entries {
			...
			// wuxingyi: 如果vlog不是每次都sync的话，那么比较小的key就不需要落到vlog了，直接跟key一起写sm即可，因为vlog记了也没用啊，不sync的话，vlog数据也会丢，lsm的数据应该也会丢，大家一起丢呗，结果就是完全丢了。
			// wuxingyi: 如果value较大，那么这个比较大的value放到lsm就不合适了，所以虽然不会对vlog做sync，但还是先记一下vlog，万一没丢最好，也不会引起过大的lsm。
