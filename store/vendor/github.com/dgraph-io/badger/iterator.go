...
func (item *KVItem) prefetchValue() {
	item.err = item.kv.yieldItemValue(item, func(val []byte) {
		...
		copy(buf, val)
		item.val = buf
		item.status = prefetched //wuxingyi: 这个key是被prefetch出来的
	})
}
...
//   }
//   itr.Close()
// wuxingyi: 创建一个Iterator之后，遍拥有了memtable和sstable的iterator。
func (s *KV) NewIterator(opt IteratorOptions) *Iterator {
	tables, decr := s.getMemTables()
	defer decr()
	var iters []y.Iterator //wuxingyi: 这里使用的是y.Iterator这个interface.
	for i := 0; i < len(tables); i++ {
		iters = append(iters, tables[i].NewUniIterator(opt.Reverse))
	}
	iters = s.lc.appendIterators(iters, opt.Reverse) // This will increment references. //wuxingyi: 给跪了，这里加的就是持久化存储之后的sst文件啊。
	res := &Iterator{
		kv:   s,
